			...
			if (args.length==0){
				...
				for(int k=0;k<(args.length);k++){
					...
					if(args[k].equals("-target")){//almacenamos el valor del target en la hash table
						stage=stage1;
						if(args[k+1].equals("scan")){stage1=1;}
						if(args[k+1].equals("parse")){stage1=2;}
						if(args[k+1].equals("ast")){stage1=3;}
						if(args[k+1].equals("semantic")){stage1=4;}
						if(args[k+1].equals("irt")){stage1=5;}
						if(args[k+1].equals("codegen")){stage1=6;}
						if(stage1>stage){Menu.put("-target", args[k+1]);}
					}
